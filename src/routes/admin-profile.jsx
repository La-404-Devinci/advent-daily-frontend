...
async function grantPoints(userId, challengeId, token, grant = true) {
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/granters/${grant ? "grant" : "ungrant"}`,
    {
      method: "POST",
      headers: {
...
export default function AdminProfile() {
  ...
  const handleSubmit = async () => {
    try {
      const { response, grant } = await grantPoints(
        userUuid,
      ...
      if (!response?.response[0]?.success) {
        toast.error(
          "Une erreur est survenue lors de la créditation du joueur",
          {
      ...
      toast.success(
        `Les points du joueur ont été ${
          grant ? "crédités" : "révoqués"
        } avec succès !`,
        {
    ...
    <Layout>
      ...
      <div className="flex flex-col justify-between gap-8 p-6 mt-16 min-h-[calc(100svh-4rem)] w-full">
            ...
            <MiniCard className="flex gap-3 items-center p-3 rounded-2xl">
              ...
              <div className="flex flex-col">
                <h2 className="text-2xl font-bold">
                  {profiles[userUuid]?.user?.username}
                </h2>
        ...
        <Button
          ...
          disabled={
            !selectedChallenge || !!userChallengesHashMap[selectedChallenge?.id]
          }
        >
          Créditer les points
        </Button>
          ...
          {userChallengesHashMap[selectedChallenge?.id] ? (
            ...
            >
              <div className="w-full py-4 px-6">
                <h3 className="text-lg text-gray-50 font-semibold">
                  Êtes-vous sûr de vouloir révoquer les points de ce joueur ?
                </h3>
                <p className="mt-2 text-sm text-gray-300">
                  Révoquer les points est une action qui a un impact le
                  classement.
                </p>
              </div>
            ...
            >
              <div className="w-full py-4 px-6">
                ...
                <p className="mt-2 text-sm text-gray-300">
                  Créditer un joueur est une action qui a un impact le
                  classement.
                </p>
              </div>
              <div className="py-4 px-4 w-full border-t border-b border-blue-950">
